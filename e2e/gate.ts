 ...
 *     nothing. Nearly everything this lab teaches — the query walkthrough, the
 *     two server views, the recovered record — renders only after a book is
 *     selected and a query generated. `.walk-step` in particular starts at
 *     `opacity: 0` and is revealed by a `.visible` class added per step, so a
 *     scan taken too early measures steps that have not appeared yet.
 *
...
export async function boot(page: Page, theme: 'dark' | 'light'): Promise<void> {
  ...
  await expect(page.locator('#catalog-grid [role="option"]')).not.toHaveCount(0);
 ...
 * plausible offender: it lays the catalog beside the query panel and prints
 * long index vectors and per-server bit sets.
 */
 ...
 *
 * The whole point of the lab is what the two servers each see, and none of it
 * exists until a book is chosen and a query generated. Each `.walk-step` is
 * revealed by a `.visible` class, so the gate waits for the last step to carry
 * it rather than scanning a half-revealed walkthrough.
 */
export async function driveAllStates(page: Page, theme: string): Promise<void> {
  await scan(page, `${theme} / first paint`);

  // Select a book from the catalog listbox.
  const options = page.locator('#catalog-grid [role="option"]');
  await options.first().click();
  await expect(page.locator('#selected-display .sel-none')).toHaveCount(0);
  await scan(page, `${theme} / book selected`);

  // Generate the PIR query, then wait for the staged walkthrough to finish
  // revealing before scanning it.
  await page.locator('#btn-generate').click();
  const steps = page.locator('.walk-step');
  await expect(steps.first()).toBeVisible();
  await expect(steps.last()).toHaveClass(/visible/, { timeout: 30_000 });
  await expect(page.locator('#server-views')).not.toBeEmpty();
  await scan(page, `${theme} / query generated`);

  // Run again with a different book, so the server views hold different sets
  // and the "same-looking query, different record" point is on screen.
  await options.nth(1).click();
  await page.locator('#btn-generate').click();
  await expect(steps.last()).toHaveClass(/visible/, { timeout: 30_000 });
  await scan(page, `${theme} / second query`);

  await page.locator('#btn-again').click();
  await expect(steps.last()).toHaveClass(/visible/, { timeout: 30_000 });
  await scan(page, `${theme} / run again`);
}
