 ...
 *    files the node under `incomplete`. The index-set chips are
 *    `color-mix(..., transparent)` washes over the surface, so their text
 ...
 *    which is not the colour that lands on screen. `.walk-step` starts at
 *    `opacity: 0` and is brought to 1 by a `.visible` class, so a step not yet
 *    revealed reads as fully faded.
 *
 ...
 *    the comparison table sits in an `overflow-x: auto` wrap and the index-set
 *    rows are long, so at 380px much of them is clipped away. Content scrolled past the client box is not dimmed or partly
